...
void SX1508::pinDir(byte pin, byte inOut)
{
	...
	unsigned int tempRegDir = readByte(REG_DIR
		);
	if (modeBit)	
		tempRegDir |= (1<<pin);
	else
...
void SX1508::writePin(byte pin, byte highLow)
{
	...
	{
		unsigned int tempRegData = readByte(REG_DATA);
	...
	{
		unsigned int tempPullUp = readByte(REG_PULL_UP);
		unsigned int tempPullDown = readByte(REG_PULL_DOWN);
...
{
	unsigned int tempRegDir = readWord(REG_DIR);
	
	if (tempRegDir & (1<<pin))	// If the pin is an input
	{
		unsigned int tempRegData = readWord(REG_DATA);
...
void SX1508::ledDriverInit(byte pin, byte freq /*= 1*/, bool log /*= false*/)
{
	...
	tempWord = readByte(REG_INPUT_DISABLE);
	tempWord |= (1<<pin);
	writeByte(REG_INPUT_DISABLE, tempWord);
	
	// Disable pull-up
	// Writing a 0 to the pin bit will disable that pull-up resistor
	tempWord = readByte(REG_PULL_UP);
	tempWord &= ~(1<<pin);
	writeByte(REG_PULL_UP, tempWord);
	
	// Set direction to output (REG_DIR)
	tempWord = readByte(REG_DIR);
	tempWord &= ~(1<<pin); // 0=output	
	writeByte(REG_DIR, tempWord);
	...
	tempWord = readByte(REG_LED_DRIVER_ENABLE);
	tempWord |= (1<<pin);
	writeByte(REG_LED_DRIVER_ENABLE, tempWord);
	
	// Set REG_DATA bit low ~ LED driver started
	tempWord = readByte(REG_DATA);
	tempWord &= ~(1<<pin);
	writeByte(REG_DATA, tempWord);
}
...
void SX1508::keypad(byte rows, byte columns, unsigned int sleepTime, byte scanTime, byte debounceTime)
{
	...
	tempWord = readWord(REG_DIR);
	for (int i=0; i<rows; i++)
		tempWord &= ~(1<<i);
	for (int i=8; i<(columns * 2); i++)
		tempWord |= (1<<i);
	writeWord(REG_DIR, tempWord);
...
unsigned int SX1508::readKeypad()
{
	return readKeyData();
}

unsigned int SX1508::readKeyData()
{
	return (0xFFFF ^ readWord(REG_KEY_DATA));
}

byte SX1508::getRow(unsigned int keyData)
{
...
byte SX1508::getCol(unsigned int keyData)
{
...
{
	unsigned int debounceEnable = readWord(REG_DEBOUNCE_ENABLE);
	debounceEnable |= (1<<pin);
	writeWord(REG_DEBOUNCE_ENABLE, debounceEnable);
}
...
void SX1508::enableInterrupt(byte pin, byte riseFall)
{
	...
	unsigned int tempWord = readWord(REG_INTERRUPT_MASK);
	tempWord &= ~(1<<pin);	// 0 = event on IO will trigger interrupt
	writeWord(REG_INTERRUPT_MASK, tempWord);
	...
	if (pin >= 8)	senseRegister = REG_SENSE_HIGH;
	else			senseRegister = REG_SENSE_HIGH;
...
unsigned int SX1508::interruptSource(bool clear /* =true*/)
{
	unsigned int intSource = readWord(REG_INTERRUPT_SOURCE);
	if (clear)
		writeWord(REG_INTERRUPT_SOURCE, 0xFFFF);	// Clear interrupts
	return intSource;
}
